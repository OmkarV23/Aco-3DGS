...
class SceneSampler:
    ...
    def __init__(self, num_dense_rays, num_sparse_rays, max_distance, beamwidth=None, device='cuda'):
        self.num_dense_rays = num_dense_rays
        self.num_sparse_rays = num_sparse_rays
        self.max_distance = max_distance
        self.beamwidth = beamwidth
        # self.dists_scene = dists_scene.detach().cpu().numpy()
        ...
        if num_sparse_rays is None and beamwidth is not None:
            ...
            #TODO: Ask about the significance of doing this. Remember that in paper, the slice is along the z-axis
        ...
        # self.scene_limits = np.array([[-self.max_radius, self.max_radius],[-self.max_radius, self.max_radius], [self.dists_scene[0], self.dists_scene[-1]]])
    ...
    def ellipsoidal_sampling(self, radii, tx_pos, rx_pos, num_rays, scene_bounds,
        ...
        if transmit_from_tx:
            dist_to, _ = ray_ellipse_intersection(a=a[:, None], b=b[:, None], c=c[:, None], vec_o=tx_origin,
                                                  vec_dir=dir_to)
            # X = O + tD
            # Check equation 15 in the paper.
            vec_to = tx_origin[None, None, :] + dist_to[..., None]*dir_to[None, ...]  #### -----------> gives the interseaction of every ray with every ellipsoid in 3D space
        else:
